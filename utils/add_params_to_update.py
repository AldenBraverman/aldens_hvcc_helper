...
# This method is pretty copy pasta lol
    ...
    while i < len(lines):
        ...
        if '@_ADD_PARAMS_TO_UPDATE_HERE' in lines[i]:
            for param in params:
                name = param["name"]
                hash_val = param["hash"]
                updated_lines.append(f"        float _{name} = {name}Param->get();\n")
...
# Copy pasta from above, but this is for the cast param
def insert_cast_param(cpp_path, json_path):
    if not os.path.exists(cpp_path):
        print(f"Error: C++ file not found: {cpp_path}")
        sys.exit(1)
    ...
    while i < len(lines):
        updated_lines.append(lines[i])
        if '@_ADD_CAST_PARAMETERS_HERE' in lines[i]:
            for param in params:
                name = param["name"]
                updated_lines.append(f"castParameter(apvts, ParameterID::{name}, {name}Param);\n")
        i += 1
